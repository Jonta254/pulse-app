// POST /api/verdex/goals/back — back (YES) or doubt (NO) someone's personal goal
...
export async function POST(req: NextRequest) {
  if (isRateLimited(req, "verdex-goals-back", 8)) return rateLimitResponse();
  ...
  const { error: insertErr } = await client.from("verdex_goal_backs").insert({
    goal_id: goalId,
  ...
  if (insertErr) {
    ...
    console.error("[verdex/goals/back]:", insertErr.message);
