...
import { PLATFORM_FEE_BPS } from "@/lib/pulse/data";
...
// ── GET /api/pulse/goals — list open goals ────────────────────────────────────
export async function GET(req: NextRequest) {
  if (isRateLimited(req, "pulse-goals-get", 30)) return rateLimitResponse();
  ...
  if (error) {
    console.error("[pulse/goals] GET error:", error.message);
...
// ── POST /api/pulse/goals — create a goal ─────────────────────────────────────
type CreateGoalBody = {
  id: string;
...
export async function POST(req: NextRequest) {
  if (isRateLimited(req, "pulse-goals-create", 5)) return rateLimitResponse();
  ...
  const { error } = await client.from("pulse_goals").insert({
    id,
  ...
  if (error) {
    ...
    console.error("[pulse/goals] insert error:", error.message);
