// GET /api/pulse/flash — returns the current batch of flash markets (5-min to 1-hr micro markets)
...
function getLiveFlashFallback(): PulseMarket[] {
  const now = new Date();
...
export async function GET(req: NextRequest) {
  if (isRateLimited(req, "pulse-flash", 60)) return rateLimitResponse();
  ...
  const markets: PulseMarket[] = data.map((row) => ({
    id: row.id,
    title: row.title,
