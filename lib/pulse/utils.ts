...
import type { PulseBet, PulseGoal, PulseMarket, PulsePlayerStats } from "@/types/pulse";

const STORAGE_BETS = "pulse_bets_v1";
const STORAGE_GOALS = "pulse_goals_v1";
const STORAGE_LOCAL_POOLS = "pulse_local_pools_v1";

// ── Storage ──────────────────────────────────────────────────────────────────

export function loadPulseBets(): PulseBet[] {
  return loadJsonFromStorage<PulseBet[]>(STORAGE_BETS, []);
}

export function savePulseBets(bets: PulseBet[]) {
  saveJsonToStorage(STORAGE_BETS, bets.slice(0, 200));
}

export function loadPulseGoals(): PulseGoal[] {
  return loadJsonFromStorage<PulseGoal[]>(STORAGE_GOALS, []);
}

export function savePulseGoals(goals: PulseGoal[]) {
...
export function applyLocalPools(markets: PulseMarket[]): PulseMarket[] {
...
export function addBetToLocalPools(marketId: string, position: "yes" | "no", amount: number, baseMarket: PulseMarket) {
  const pools = loadLocalPools();
...
export function calcPlayerStats(bets: PulseBet[]): PulsePlayerStats {
